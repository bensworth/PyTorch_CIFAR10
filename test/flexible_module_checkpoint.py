"""
This is an adapation of the Lightning source code that saves checkpoints in different periods and stages
of training.
"""
...
class FlexibleModelCheckpoint(Callback):
    r"""
    ...
    Args:
        dirpath: directory to save the model file.
            ...
            Example::
                ...
                >>> checkpoint_callback = FlexibleModelCheckpoint(dirpath='my/path/')
        ...
        filename: checkpoint filename. Can contain named formatting options to be auto-filled.
            ...
            Example::
                ...
                >>> checkpoint_callback = FlexibleModelCheckpoint(
                ...     dirpath='my/path',
        ...
        every_n_val_epochs: Number of validation epochs between checkpoints.
            ...
            Setting both ``FlexibleModelCheckpoint(..., every_n_val_epochs=V)`` and
    ...
    Note:
        For extra customization, FlexibleModelCheckpoint includes the following attributes:
    ...
    Example::
        ...
        >>> from pytorch_lightning.callbacks import FlexibleModelCheckpoint
    ...
    def __init__(
        ...
        train_step_region: List[Tuple[int, int]] = None,
        ...
        self.__init_triggers(every_n_train_steps, every_n_val_epochs, period, train_step_region)
    ...
    def on_train_batch_end(
        ...
        in_region = any(left <= step <= right for left, right in self._train_step_region)
        skip_batch = skip_batch and (not in_region)
#        print(step, in_region, skip_batch)
        if skip_batch:
            return
    ...
    def save_checkpoint(self, trainer: 'pl.Trainer', unused: Optional['pl.LightningModule'] = None) -> None:
        ...
        if unused is not None:
            rank_zero_deprecation(
                "`FlexibleModelCheckpoint.save_checkpoint` signature has changed in v1.3. The `pl_module` parameter"
    ...
    def __validate_init_configuration(self) -> None:
        ...
        if self._every_n_train_steps > 0 and self._every_n_val_epochs > 0:
            raise MisconfigurationException(
                ...
                ' and every_n_val_epochs={every_n_val_epochs}.'
        ...
        if self.monitor is None:
            ...
            if self.save_top_k not in (None, -1, 0):
                raise MisconfigurationException(
                    f'FlexibleModelCheckpoint(save_top_k={self.save_top_k}, monitor=None) is not a valid'
                    ' configuration. No quantity for top_k to track.'
                )
            if self.save_last:
                rank_zero_warn(
                    'FlexibleModelCheckpoint(save_last=True, save_top_k=None, monitor=None) is a redundant configuration.'
                    ' You can save the last checkpoint with FlexibleModelCheckpoint(save_top_k=None, monitor=None).'
                )
            if self.save_top_k == -1 and self.save_last:
                rank_zero_info(
                    'FlexibleModelCheckpoint(save_last=True, save_top_k=-1, monitor=None)'
    ...
    def __init_triggers(
        ...
        if period is not None:
            rank_zero_deprecation(
                'Argument `period` in `FlexibleModelCheckpoint` is deprecated in v1.3 and will be removed in v1.5.'
                ' Please use `every_n_val_epochs` instead.'
            )
            self._every_n_val_epochs = period

        self._train_step_region = train_step_region if train_step_region is not None else []
    ...
    def period(self) -> Optional[int]:
        rank_zero_deprecation(
            'Property `period` in `FlexibleModelCheckpoint` is deprecated in v1.3 and will be removed in v1.5.'
            ' Please use `every_n_val_epochs` instead.'
        )
        return self._period
    ...
    def save_function(self) -> Optional[Callable]:
        rank_zero_deprecation(
            'Property `save_function` in `FlexibleModelCheckpoint` is deprecated in v1.3 and will be removed in v1.5.'
    ...
    def format_checkpoint_name(self, metrics: Dict[str, _METRIC], ver: Optional[int] = None) -> str:
        ...
        Example::
            ...
            >>> ckpt = FlexibleModelCheckpoint(dirpath=tmpdir, filename='{epoch}')
            >>> os.path.basename(ckpt.format_checkpoint_name(dict(epoch=0)))
            'epoch=0.ckpt'
            >>> ckpt = FlexibleModelCheckpoint(dirpath=tmpdir, filename='{epoch:03d}')
            >>> os.path.basename(ckpt.format_checkpoint_name(dict(epoch=5)))
            'epoch=005.ckpt'
            >>> ckpt = FlexibleModelCheckpoint(dirpath=tmpdir, filename='{epoch}-{val_loss:.2f}')
            >>> os.path.basename(ckpt.format_checkpoint_name(dict(epoch=2, val_loss=0.123456)))
            'epoch=2-val_loss=0.12.ckpt'
            >>> ckpt = FlexibleModelCheckpoint(dirpath=tmpdir,
            ... filename='epoch={epoch}-validation_loss={val_loss:.2f}',
            ... auto_insert_metric_name=False)
            >>> os.path.basename(ckpt.format_checkpoint_name(dict(epoch=2, val_loss=0.123456)))
            'epoch=2-validation_loss=0.12.ckpt'
            >>> ckpt = FlexibleModelCheckpoint(dirpath=tmpdir, filename='{missing:d}')
            >>> os.path.basename(ckpt.format_checkpoint_name({}))
            'missing=0.ckpt'
            >>> ckpt = FlexibleModelCheckpoint(filename='{step}')
    ...
    def _add_backward_monitor_support(self, trainer: 'pl.Trainer') -> None:
        ...
        if deprecation_warning:
            warning_cache.warn(
                "Relying on `self.log('val_loss', ...)` to set the FlexibleModelCheckpoint monitor is deprecated in v1.2"
                " and will be removed in v1.4. Please, create your own `mc = FlexibleModelCheckpoint(monitor='your_monitor')`"
    ...
    def _validate_monitor_key(self, trainer: 'pl.Trainer') -> None:
        ...
        if self.monitor is not None and not self._is_valid_monitor_key(metrics):
            m = (
                f"FlexibleModelCheckpoint(monitor='{self.monitor}') not found in the returned metrics:"
                f" {list(metrics.keys())}. "
